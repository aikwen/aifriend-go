...
func TestEinoEmbedQuery(t *testing.T) {
	...
	svc, err := NewEinoSvc(ctx, EinoConfig{
		APIKey:     apiKey,
		BaseURL:    apiBase,
	...
	if err != nil {
		t.Fatalf("new eino embedding svc failed: %v", err)
	}

	vec, err := svc.EmbedQuery(ctx, "阿里云百炼平台是什么？")
	if err != nil {
		t.Fatalf("eino embed query failed: %v", err)
	}
